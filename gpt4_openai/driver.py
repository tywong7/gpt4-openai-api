...
class ChatGptDriver:
    ...
    def __init_browser(self) -> None:
        ...
        try:
            self.driver = uc.Chrome(options=options, headless=self.__headless)
        ...
        # To avoid clicking on the "Get started" button
        local_storage_data = {
            'oai/apps/hasSeenOnboarding/chat': '"2023-11-11"',
            'oai/apps/hasSeenGizmoOnboarding': 'true',
        }
        # Insert each key-value pair into local storage
        for key, value in local_storage_data.items():
            # JavaScript to set the item in local storage
            js_command = f"window.localStorage.setItem('{key}', '{value}');"
    ...
    def generate_image(self, prompt: str) -> str:
        ...
        for _ in range(5):
            try:
                return self.driver.find_element(By.XPATH, "//img[@alt='Generated by DALL·E']").get_attribute('src')
            except Exception as e:
                print(e)
                self.__sleep(0.5)
                pass
    ...
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        self.driver.save_screenshot('send_message.png')
    ...
    def chat(self, message: str, stream: bool = False) -> dict:
        ...
        if responses:
            ...
            if 'text-red' in response.get_attribute('class'):
                self.logger.debug('Response is an error')
                raise ValueError(response.text)
        responses = self.driver.find_elements(*chatgpt_small_response)
        ...
        if len(responses) == 0: # There was some problem, no response was generated
            ...
            try:
                ...
                self.driver.find_element(By.XPATH, request_limit_xpath)
